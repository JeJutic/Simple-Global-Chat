...
@Slf4j
public class AnalyzeRequestControllerImpl implements AnalyzeRequestController {

    private final FullMessageAnalyzer messageAnalyzer;

    @Autowired
    public AnalyzeRequestControllerImpl(FullMessageAnalyzer messageAnalyzer) {
        this.messageAnalyzer = messageAnalyzer;
    }

    @Override
    @PostMapping("/analyze")
    public ResponseEntity<Integer> findRating(@Valid @RequestBody Message message, Errors errors) {
